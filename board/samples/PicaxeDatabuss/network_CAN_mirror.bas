...
symbol CS = C.7  : symbol CS_PIN = pinC.7
symbol MOSI= C.6 : symbol MOSI_PIN = pinC.6
symbol MISO = C.5   : symbol MISO_PIN = pinC.5
symbol SCK = C.2 :symbol SCK_PIN = pinC.2
...
symbol motorPins  = b10
symbol stopCounter = b11
...
main:
	...
	gosub update ' must run each loop for motorcontrol to work
	if command > 0 then
		gosub returnValues ' if we got a measurement request from CAN ' measured in update
		gosub resetValues
	end if
	
	 ' used to stop motors from moving to much
...
update:
	...
	if command = readKey then
		if value = 0 then
			...
			'else if state = 2 then   			' need this pin pin for CAN interface ' temperature
...
' when we have measured a value we need to broadcast it
returnValues:
...
eventFromCAN:
	if command > 0 then
		gosub cmdToSerial
		gosub update
		gosub returnValues ' if we got a measurement request from CAN ' measured in update that is called 
		gosub resetValues
		pause cableTimeout
	end if
	return 
