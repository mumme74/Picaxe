...
main:
	'sertxd("in main\r\n")
	gosub errorCheck
	gosub pollInBox ' calls update implicitly if a new event comes from CAN
...
checkBlinkState:
	if command = leftKey or command = rightKey or command = "F" then
		if  state = 48 and blinkerSide <> 0 then
			blinkerSide = 0 ' turn blinker off
		else if state > 48 and blinkerSide <> command then
			blinkerSide = command ' turn blinker on
			toggleCounter = toggleAt
			toggleState = 0
		end if
	end if
	return

' updates the outputs from this board
update:
	...
	if command = leftKey then
		pinB.5 = state
	else if command = rightKey then
		pinB.1 = state
