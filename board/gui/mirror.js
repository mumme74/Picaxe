...
//   }
  ...
  function Mirror(picaxeController, parentNodeID, protocolID, rightOriented, scale, uniqueDomID){
    ...
    addOnDOMLoad(function(){
      ...
      // ladda HTML
      var svg = loadTextFile("mirror_template.htm");
  ...
  MirrorPosition = function(ownerMirror){
    ...
    this.setXPos = function(newX){
      ...
      x = newX;
      var upArrow = ownerMirror._getNode("up_arrow");
      var downArrow = ownerMirror._getNode("down_arrow");
   
      var upScale = x/127; // faktor att multiplicera med
      var downScale = (255 - x) / 127;
      // up arrow
      var height = parseInt(up.height * upScale)
      upArrow.style.height = height + "px";
      upArrow.style.top = (up.top + (up.height - height)) + "px";
      
      upArrow.setAttribute("y", (up.top + (up.height - height)));
      upArrow.setAttribute("height", height);
      //down arrow
  ...
  MirrorBlinker = function(ownerMirror){
    ...
    this.setState = function(newState){
      ...
      ownerMirror._getNode("blinksymbolfield1").style.fill = state ? "#FFC321" : "white";
    }
    this.getState = function(){ return state; }
  ...
  MirrorSafeLight = function(ownerMirror) {
    ...
    this.setState = function(newState){
      ...
      ownerMirror._getNode("safelightsymbolfield1").style.fill = state ? "#FFFFC1" : "white";
      ownerMirror._getNode("safelightsymbolfield2").style.fill = state ? "#FFFFC1" : "white";
    }
    this.getState = function(){ return state; }
  ...
  MirrorHeater = function(ownerMirror){
    ...
    this.setState = function(newState){
      state = newState;
      ownerMirror._getNode("heatsymbol").style.fill = state ? "#FF796B" : "#7CC4FF";
    }
  ...
  MirrorThermometer = function(ownerMirror){
    ...
    this.setValue = function(newValue){
      ...
      if (temp >= 12 && temp <= 32){
        ...
        scale.setAttribute("height", height);
      }
